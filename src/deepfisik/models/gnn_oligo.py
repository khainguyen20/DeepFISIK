...
class GNNOligo(torch.nn.Module):
    def __init__(self, model_params):
        super(GNNOligo, self).__init__()
        ...
        if noClasses == 2:
            self.out = Linear(int(node_embedding_size/4), 1)
        elif noClasses > 2:
            self.out = Linear(int(node_embedding_size/4), noClasses)
    ...
    def forward(self, x, edge_index,edge_attr, batch_index, device):
        ...
        if self.noClasses == 2:
            u = self.out(u)
        elif self.noClasses > 2:
            u = self.out(u)
        edge_attr = self.edge_decode(edge_attr)
        return x, edge_attr, u, #gelu1, gelu2#, attention_coeffs
