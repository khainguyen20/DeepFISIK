...
def train(device,train_loader, model, time_step, optimizer, criterion):
    ...
    for train_data in train_loader:  # Iterate in batches over the training dataset.
        ...
        if torch.cuda.device_count() > 1:
            ...
            node_out, edge_out, global_out,DC,AP1,AP2,AP3,DR1,DR2,DR3,RD,LF = model(data.x.to(device),data.edge_index.long().to(device),data.edge_attr.to(device), data.batch.to(device), device)
            ...
            true_AP3 = true_AP3.reshape(true_AP2.size()[0],1)
            true_DR3 = data.DR3.float().to(device)*20*time_step  # getting the true DR values
            true_DR3 = true_DR3.reshape(true_DR2.size()[0],1)
        ...
        lossDC,lossAP1,lossAP2,lossAP3,lossDR1,lossDR2,lossDR3,lossRD,lossLF,loss = criterion([true_DC,true_AP1,true_AP2,true_AP3,true_DR1,true_DR2,true_DR3,true_RD,true_LF],[DC,AP1,AP2,AP3,DR1,DR2,DR3,RD,LF]) # calculate loss for DC, AP, DR, and summed losses
    ...
    return np.mean(losses), np.mean(lossesDC), np.mean(lossesAP1),np.mean(lossesAP2),np.mean(lossesAP3), np.mean(lossesDR1),np.mean(lossesDR2),np.mean(lossesDR3),np.mean(lossesRD),np.mean(lossesLF)#, np.asarray(residuals_trainDC).flatten(), np.asarray(residuals_trainAP).flatten(), np.asarray(residuals_trainDR).flatten()
...
def train_testTetramers(
    ...
    for epoch in range(n_epochs):
        ...
        lossAP3 += [trainResults[4]]
        lossDR1 += [trainResults[5]]
        lossDR2 += [trainResults[6]]
        lossDR3 += [trainResults[7]]
        ...
        train_scoreAP3 += [trainAccuracy[4]]
        train_scoreDR1 += [trainAccuracy[5]]
        train_scoreDR2 += [trainAccuracy[6]]
        train_scoreDR3 += [trainAccuracy[7]]
        ...
        test_scoreAP3 += [testAccuracy[4]]
        test_scoreDR1 += [testAccuracy[5]]
        test_scoreDR2 += [testAccuracy[6]]
        test_scoreDR3 += [testAccuracy[7]]
        ...
        results = pd.DataFrame({
            'loss': loss, 'lossDC':lossDC,'lossAP1':lossAP1,'lossAP2':lossAP2,'lossAP3':lossAP3,'lossDR1':lossDR1,'lossDR2':lossDR2,'lossDR3':lossDR3,'lossRD':lossRD,'lossLF':lossLF,
            'train_score': train_score, 'train_scoreDC': train_scoreDC,'train_scoreAP1': train_scoreAP1,'train_scoreAP2': train_scoreAP2,'train_scoreAP3': train_scoreAP3,'train_scoreDR1': train_scoreDR1,'train_scoreDR2': train_scoreDR2,'train_scoreDR3': train_scoreDR3,'train_scoreRD':train_scoreRD,'train_scoreLF':train_scoreLF,
            'test_score': test_score, 'test_scoreDC': test_scoreDC, 'test_scoreAP1': test_scoreAP1,'test_scoreAP2': test_scoreAP2,'test_scoreAP3': test_scoreAP3, 'test_scoreDR1': test_scoreDR1,'test_scoreDR2': test_scoreDR2,'test_scoreDR3': test_scoreDR3,'test_scoreRD':test_scoreRD,'test_scoreLF':test_scoreLF,
